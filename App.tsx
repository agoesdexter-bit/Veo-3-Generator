...
export const App: React.FC = () => {
  ...
  const [newPrompt, setNewPrompt] = useState(
    'Ibu hamil terlihat santai rebahan di sofa sambil main HP',
  );
  ...
  const genericErrorHandler = (error: unknown) => {
    ...
    let errorMessages = [
      ...
      'This could be due to a network issue, API limitations, or content restrictions.',
    ...
    if (errorToParse) {
      try {
        const errorJson = JSON.parse(errorToParse);
        if (errorJson.error?.message) {
          errorMessages = ['Video generation failed:', errorJson.error.message];
          if (errorJson.error.status) {
            errorMessages.push(`Status: ${errorJson.error.status}`);
          }
        } else {
          // It's JSON but not the expected format
          errorMessages = ['An unexpected error occurred:', errorToParse];
        }
      } catch (e) {
    ...
    setGenerationError(errorMessages);
  };
        ...
        <div className="mx-auto max-w-[1080px]">
          <header className="p-6 md:p-8 text-center">
            ...
            <p className="text-gray-400 mt-2 text-lg">
              Generate your own videos or select one to create variations.
            </p>
          </header>
          <main className="px-4 md:px-8 pb-8">
            <section className="mb-10 p-6 bg-gray-800 rounded-lg shadow-xl">
              ...
              <textarea
                className="w-full bg-gray-900 border border-gray-700 rounded-lg p-3 text-gray-200 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-shadow duration-200 resize-y"
                rows={4}
                value={newPrompt}
